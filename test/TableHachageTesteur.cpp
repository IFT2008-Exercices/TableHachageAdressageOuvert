...
TEST(TableHachage, constructeurDefaut) {
    typedef TableHachage<string, double, HacheurQuadStr1> TableT ;
    EXPECT_NO_THROW( TableT t );
}

TEST(TableHachage, insererNoThrow) {
    TableHachage<string, double, HacheurQuadStr1> table;
...
TEST_F(TableHachageTest, enleverThrowSiPasPresent) {
    TableHachage<string, double, HacheurQuadStr1> table2;
...
TEST_F(TableHachageTest, afficherOk) {
    string attendu = "{(fraise,16.4),(poire,9.45),(raisin,9),(orange,12.4),(pomme,15.3),(nom de fruit inconnu mais tres savoureux,55),(mangue,7.6),(banane,7.23),}" ;
    ostringstream os ;
    table.afficher(os) ;
    EXPECT_EQ(attendu, os.str() ) ;
}
...
TEST_F(TableHachageTest, rehacherOk) {
    ...
    EXPECT_TRUE(table.contient("raisin"));
    EXPECT_TRUE(table.contient("nom de fruit inconnu mais tres savoureux"));
    EXPECT_TRUE(table.taille() == 8) ;
}

TEST(TableHachageTestIndv, fluxEnleverAjouterOk) {
    TableHachage<int, int, HacheurQuadInt1> table;
    srand(time(NULL));
    int v;
